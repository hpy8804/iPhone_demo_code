...
{
    NSLog(@"self.previous:%@", self.previous);
    NSLog(@"self.current:%@", self.current);
    NSDecimalNumber *result = [self.current decimalNumberByAdding:self.previous];
    NSLog(@"result:%@", result);
    NSLog(@"[NSDecimalNumber decimalNumberWithMantissa:1 exponent:40 isNegative:NO]:%@", [NSDecimalNumber decimalNumberWithMantissa:1 exponent:40 isNegative:NO]);
