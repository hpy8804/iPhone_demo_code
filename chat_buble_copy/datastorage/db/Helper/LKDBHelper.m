...
- (void)insertAll:(NSArray *) arr {
    [NSThread detachNewThreadSelector:@selector(insertDo:) toTarget:self withObject:arr];
}

- (void) insertDo:(NSArray *) arr {
    for (NSObject *model in arr) {
...
-(void)readAll:(Class) cls where:(NSString *)where {
    ...
    [str appendString:@"readed=0"];
    [self updateToDB:cls set:@"readed=1" where:str];
}

-(NSInteger)getNoreadCount:(Class)cls where:(NSString *) where {
    NSMutableString * str=[NSMutableString string];
    if(where!=nil)
    {
       [str appendString:where];
    ...
    [str appendString:@"readed=0"];
   return [self rowCount:cls where:str];
}
